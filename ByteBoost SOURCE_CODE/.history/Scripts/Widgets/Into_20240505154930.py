...
class DateTimeFrame():
    ...
    def create_widgets(self, parent):
        self.day_label = ctk.CTkLabel(parent, text_color=self.SETTINGS["FONT_COLOR"], font=(FONT_FAMILY[0], 95))
        self.day_label.place(relx=0.07, rely=0.01, anchor="nw")

        self.weekday_label = ctk.CTkLabel(parent, text_color=self.SETTINGS["FONT_COLOR"], font=(FONT_FAMILY[0], 20))
        self.weekday_label.place(relx=0.5, rely=0.12)

        self.time_label = ctk.CTkLabel(parent, text_color=self.SETTINGS["FONT_COLOR"], font=(FONT_FAMILY[0], 35))
...
class QuotesFrame():
    ...
    def create_widgets(self, parent):
        title_label = ctk.CTkLabel(parent, text_color=self.SETTINGS["FONT_COLOR"], text="Dijkstra's Law", font=(FONT_FAMILY[2], 20))
        title_label.place(relx=0.07, rely=0.5)

        quote_label = ctk.CTkLabel(parent, text_color=self.SETTINGS["FONT_COLOR"], text="Picking up a new game is like learning a new language: you fumble with the controls and hope you don't embarrass yourself in front of NPCs", justify="left", font=(FONT_FAMILY[2], 12), wraplength=250)
