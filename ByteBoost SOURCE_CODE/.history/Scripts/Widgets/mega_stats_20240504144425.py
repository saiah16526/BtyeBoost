...
class Statistics(ctk.CTkScrollableFrame):
    ...
    def create_date_stats(self):
        ...
        ctk.CTkLabel(self.name_frame, fg_color=font_color,text="LESSON", font=(font_family[0], 13)).pack(fill="x", expand=True)
        ctk.CTkLabel(self.category_frame, fg_color=font_color,text="CATEGORY", font=(font_family[0], 13)).pack(fill="x", expand=True)
        ctk.CTkLabel(self.from_frame, fg_color=font_color,text="FROM", font=(font_family[0], 13)).pack(fill="x", expand=True)
        ctk.CTkLabel(self.to_frame, fg_color=font_color,text="TO", font=(font_family[0], 13)).pack(fill="x", expand=True)
        ctk.CTkLabel(self.duration_frame, fg_color=font_color,text="DURATION", font=(font_family[0], 13)).pack(fill="x", expand=True)
