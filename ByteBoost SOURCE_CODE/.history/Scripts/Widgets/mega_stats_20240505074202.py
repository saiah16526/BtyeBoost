...
class Statistics(ctk.CTkScrollableFrame):
    ...
    def update_stats(self, name, category, start, to, duration, color):
       label_1 =  ctk.CTkLabel(fg_color=color,master=self.name_frame, text=name, font=(font_family[0], 13),width=150)
       label_1.pack(fill="x", padx=1)
       label_2 =  ctk.CTkLabel(fg_color=color,master=self.category_frame, text=category, font=(font_family[0], 13),width=130)
       label_2.pack(fill="x", padx=1)
       label_3 =  ctk.CTkLabel(fg_color=color,master=self.from_frame, text=start, font=(font_family[0], 13),width=70)
       label_3.pack(fill="x", padx=1)
       label_4 =  ctk.CTkLabel(fg_color=color,master=self.to_frame, text=to, font=(font_family[0], 13),width=70)
       label_4.pack(fill="x", padx=1)
       label_5 =  ctk.CTkLabel(fg_color=color,master=self.duration_frame, text=duration, font=(font_family[0], 13),width=40)
       label_5.pack(fill="x", padx=1)
