...
class Statistics(ctk.CTkScrollableFrame):
    def __init__(self, master):
        super().__init__(master, fg_color=SETTINGS["background_color"], border_width=SETTINGS["border_width"], corner_radius=SETTINGS["border_radius"], border_color=SETTINGS["border_color"])
        self.startup_load()
    ...
    def update_stats(self, label_frames, contents, color):
        for lf in label_frames:
            label_text = lf.winfo_children()[0]["text"].lower()
            label_width = lf.winfo_children()[0]["width"]

            label = ctk.CTkLabel(lf, fg_color=color, font=(SETTINGS["font_family"][0], 13), width=label_width)
            label.pack(fill="x", padx=1)
            label.propagate(flag=False)

            # Set label text based on contents dictionary
            label.config(text=contents.get(label_text, ""))
