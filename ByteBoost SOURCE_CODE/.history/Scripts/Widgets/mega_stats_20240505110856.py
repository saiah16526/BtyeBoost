...
class Statistics(ctk.CTkScrollableFrame):
    ...
    def create_date_stats(self, date, details):
        ...
        def create_and_pack_frames(frame):
            frame_names = ["name_frame", "category_frame", "from_frame", "to_frame", "duration_frame"]
            frames = {}

            for name in frame_names:
                new_frame = ctk.CTkFrame(frame, corner_radius=0)
                new_frame.pack(fill="x", expand=True, side="left")
                frames[name] = new_frame

            return frames

        # Usage:
        self.frames = self.create_and_pack_frames(frame)
