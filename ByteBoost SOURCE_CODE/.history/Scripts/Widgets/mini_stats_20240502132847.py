...
class StatsWrapper(ctk.CTkFrame):
    ...
    def create_progress_display(self):
        ...
        canvas_stats.place(relx=0.01, rely=0.01, relwidth=0.98, relheight=0.98)
    ...
    def draw_progress_bars(self, canvas, lessons_dict):
        ...
        canvas.create_rectangle(106, 10, 108, y_increment, fill="#4fa", outline=Stats_background)

    def draw_language_bar(self, canvas, text, percentage, y_increment):
        x = 103  # Initial x position for the progress bar
        y = 5 + y_increment  # Calculate y position based on y_increment
        ...
        self.draw_text(canvas, x, y, text, anchor="ne", font=("Arial", 10), fill=border)
        ...
        self.draw_text(canvas, current_x + 35, y, f"{int(percentage)}%", font=("JetBrains Mono", 9, 'bold'), anchor="ne", fill="#0f0")
