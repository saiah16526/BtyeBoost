...
class StatsWrapper(ctk.CTkFrame):
    def __init__(self, master, border="#88aa77"):
        load_settings()
        super().__init__(master, fg_color=BACKGROUND, border_width=BORDER_WIDTH, border_color=BORDER_COLOR)  # Initialize the parent class (tk.Frame)
        self.STATS_BACKGROUND = BACKGROUND  # Set the background color
        self.parent = master
        ...
        frame = ctk.CTkFrame(self, fg_color=self.STATS_BACKGROUND, border_color=BORDER_COLOR, border_width=BORDER_WIDTH, corner_radius=BORDER_RADIUS)
    ...
    def draw_language_bar(self, canvas, text, percentage, y_increment):
        ...
        self.draw_rectangle(canvas, x + 10, y + 3.5, current_x, y + 10.5, fill=PROGRESS_THEME)
