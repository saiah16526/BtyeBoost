...
class StatsWrapper(ctk.CTkFrame):
    def __init__(self, master, border="#88aa77"):
        ...
        #image
        stats_img = ImageTk.PhotoImage(Image.open(resource_path("./Images/Icons/stats.png")).resize((45, 45)))
        
        ctk.CTkLabel(frame, text="STATISTICS", text_color=self.SETTINGS["BORDER_COLOR"],font=(self.SETTINGS["FONT_FAMILY"][1], 18)).pack(side="left", expand=True, fill="both")
    ...
    def load_lessons_data(self):
        ...
        try:
            with open(resource_path("./SaveFiles/lessons.json"), 'r') as f:
                return json.load(f)
    ...
    def draw_progress_bars(self, canvas, lessons_dict):
        ...
        canvas.create_rectangle(106, 5, 108, y_increment, fill="#4fa", outline=self.SETTINGS["BACKGROUND"])
    ...
    def draw_language_bar(self, canvas, text, percentage, y_increment):
        ...
        self.draw_text(canvas, current_x + 35, y, f"{int(percentage)}%", font=(self.SETTINGS["FONT_FAMILY"][0], 9, 'bold'), anchor="ne", fill=self.SETTINGS["PROGRESS_THEME"])
