...
class Panel:
        def __init__(self, master):
            ...
            self.frame = ctk.CTkFrame(master, fg_color=
            panel_background, border_width=width, border_color="#88aa77", corner_radius=5)
            self.toggle_button = ctk.CTkButton(self.frame, text="", corner_radius=5, command=self.toggle_panel_view)
            self.frame.place(relx=0, rely=0, relwidth=0.03, relheight=1, anchor="nw")
            self.toggle_button.place(relx=0.05, rely=0.01, anchor="nw", relwidth=0.9)
            self.lessons_panel = self.LessonGrid(self.frame, master)  # Create the lesson panel
            self.parent = master
        ...
        def update_panel_layout(self):
            if self.is_collapsed:
                ...
                # Expand the panel
                self.frame.place(relx=0, rely=0, relwidth=0.3, relheight=1, anchor="nw")
        ...
        class LessonGrid:
            def __init__(self, master, parent):
                ...
                panel_background, corner_radius=5)
            ...
            def load_lessons(self):
                ...
                lessons_dict = {}
                try:
                    with open("./Scripts/lessons.json", 'r') as r:
...
class LangPack:
    def __init__(self, master, col, row, title, subtitles, parent):
        self.frame = ctk.CTkFrame(master, fg_color=Panel_background, border_color=border_color, border_width=border_width)
        self.frame.grid(column=col, row=row, sticky="nswe")
        ctk.CTkLabel(self.frame, text=f"{title.upper()}", fg_color="#666666", text_color="#201").pack(fill="both")
    ...
    def update_selection(self, parent):
        parent.test2.lang.set(self.rad_btn.get())
