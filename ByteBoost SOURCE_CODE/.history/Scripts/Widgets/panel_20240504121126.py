...
class Panel:
        def __init__(self, master):
            ...
            Panel_background, border_width=border_width, border_color=border_color, corner_radius=border_radius)
            self.toggle_button = ctk.CTkButton(self.frame, text="", corner_radius=5, command=self.toggle_panel_view)
            self.settings_button = ctk.CTkButton(self.frame, text="SETTINGS",font=(font_family[0], 13),width=10, corner_radius=5, command=self.toggle_settings_view)
        ...
        def toggle_settings_view(self):
            print(self.parent.settings.show_panel)
