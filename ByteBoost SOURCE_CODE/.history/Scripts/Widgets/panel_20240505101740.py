...
class Panel(ctk.CTkFrame):
        def __init__(self, master):
            ...
            self.toggle_button = ctk.CTkButton(self, text="", corner_radius=5, command=self.toggle_panel_view)
            self.settings_button = ctk.CTkButton(self, text="SETTINGS",font=(font_family[0], 13),width=10, corner_radius=5, command=self.toggle_settings_view)
            self.place(relx=0, rely=0, relwidth=0.03, relheight=1, anchor="nw")
            self.toggle_button.place(relx=0.05, rely=0.01, anchor="nw", relwidth=0.9)
        ...
        def update_panel_layout(self):
            if self.is_collapsed:
                ...
                self.toggle_button.place(relx=0.99, rely=0.01, anchor="ne", relwidth=0.1)
        ...
        class LessonGrid:
            def __init__(self, master, parent):
                self.parent = parent
                self = ctk.CTkScrollableFrame(master, fg_color=
                Panel_background, corner_radius=border_radius)
                self.place(relx=0.01, rely=0.07, relwidth=0, relheight=0.93, anchor="nw")
                self.columnconfigure((0, 1), weight=1)
                ctk.CTkLabel(self, text="Available Lessons").grid(column=0, row=0, columnspan=2)
            ...
            def create_language_packs(self, lessons_dict):
                ...
                for key, value in lessons_dict.items():
                    if col > 1:
                        row += 1
                        col = 0
                    LangPack(self, col=col, row=row, title=key, subtitles=value, parent=self.parent)  # Create a language pack widget
                    col += 1  # Increment column for the next widget

class LangPack:
    def __init__(self, master, col, row, title, subtitles, parent):
        self = ctk.CTkFrame(master, fg_color=Panel_background, border_color=border_color, border_width=border_width, corner_radius=border_radius)
        self.grid(column=col, row=row, sticky="nswe")
        ctk.CTkLabel(self, text=f"{title.upper()}", fg_color=percentage_background, text_color="#201").pack(fill="both")
