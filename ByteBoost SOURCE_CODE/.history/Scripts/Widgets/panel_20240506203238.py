...
class Panel(ctk.CTkFrame):
        def __init__(self, master):
            self.SETTINGS = load_settings(master)
            super().__init__(master,fg_color=self.SETTINGS["BACKGROUND"], border_width=BORDER_WIDTH, border_color=self.SETTINGS["BORDER_COLOR"], corner_radius=BORDER_RADIUS)
            ...
            #image
            self.expand_img = ImageTk.PhotoImage(Image.open(resource_path("./Images/Icons/expand.png")).resize((40, 40)))
            self.collapse_img = ImageTk.PhotoImage(Image.open(resource_path("./Images/Icons/collapse.png")).resize((40, 40)))
            settings_img = ImageTk.PhotoImage(Image.open(resource_path("./Images/Icons/settings.png")).resize((30, 30)))
            
            self.toggle_button = ctk.CTkButton(self,fg_color=self.SETTINGS["BACKGROUND"], hover_color=self.SETTINGS["BACKGROUND"], text="",image=self.expand_img ,corner_radius=5, command=self.toggle_panel_view)
            self.settings_button = ctk.CTkButton(self, text="",fg_color=self.SETTINGS["BACKGROUND"], hover_color=self.SETTINGS["BACKGROUND"], image=settings_img,font=(self.SETTINGS["FONT_FAMILY"][0], 13),width=10, corner_radius=5, command=self.toggle_settings_view)
        ...
        class LessonGrid:
            def __init__(self, master, parent):
                ...
                self.SETTINGS["BACKGROUND"], corner_radius=BORDER_RADIUS)
                self.frame.place(relx=0.01, rely=0.07, relwidth=0, relheight=0.93, anchor="nw")
                self.frame.columnconfigure((0, 1), weight=1)
                ctk.CTkLabel(self.frame, text_color=self.SETTINGS["BORDER_COLOR"],font=(self.SETTINGS["FONT_FAMILY"][1], 14),text=f"{("Available Lessons").upper()}").grid(column=0, row=0, columnspan=2)
            ...
            def load_lessons(self):
                ...
                lessons_dict = {}
                try:
                    with open(resource_path("./SaveFiles/lessons.json"), 'r') as r:
...
class LangPack:
    def __init__(self, master, col, row, title, subtitles, parent):
        self.SETTINGS = load_settings()
        self.frame = ctk.CTkFrame(master, fg_color=self.SETTINGS["BACKGROUND"], border_color=self.SETTINGS["BORDER_COLOR"], border_width=BORDER_WIDTH, corner_radius=BORDER_RADIUS)
        self.frame.grid(column=col, row=row, sticky="nswe")
        ctk.CTkLabel(self.frame, text=f"{title.upper()}", fg_color=self.SETTINGS["PERCENTAGE_BACKGROUND"], text_color="#201").pack(fill="both")
