...
class Input():
    def __init__(self, master, text):
        self.x = 0.01
        self.y = 0.1
        label_input = ctk.CTkLabel(self, text_color=font_color, text=text, font=(font_family[2], 14), anchor="w")
        label_input.place(relx=self.x, rely=self.y)

        self.entry = ctk.StringVar()
        ctk.CTkEntry(self,textvariable=self.entry ,corner_radius=3, placeholder_text="1:45").place(relx=self.x * 2, rely=self.y)