...
class Lessons_panel():
    def __init__(self, master):
        ...
        ctk.CTkLabel(add_frame, text=f"{("New category:").upper()}", font=(font_family[0], 13)).place(relx=0.01, rely=0.34)
        new_category = ctk.CTkEntry(add_frame, placeholder_text="JavaScript", corner_radius=0, border_width=1)
        new_category.place(relx=0.3, rely=0.34)
        
        ctk.CTkLabel(add_frame, text=f"{("Lesson To Add:").upper()}", font=(font_family[0], 13)).place(relx=0.01, rely=0.53)
        new_lesson = ctk.CTkEntry(add_frame, placeholder_text="Node.js", corner_radius=0, border_width=1)
        new_lesson.place(relx=0.3, rely=0.53)
