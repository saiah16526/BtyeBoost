...
class Lessons_panel():
    def __init__(self, master):
        ...
        ctk.CTkLabel(add_frame, text=f"{("Category:").upper()}", font=(font_family[0], 13)).place(relx=0.01, rely=0.18)
        category = ctk.CTkComboBox(add_frame, corner_radius=0)
        category.place(relx=0.21, rely=0.15)
        
        ctk.CTkLabel(add_frame, text=f"{("New category:").upper()}", font=(font_family[0], 13)).place(relx=0.01, rely=0.37)
        new_category = ctk.CTkEntry(add_frame, placeholder_text="If The Category doesn't Exists", corner_radius=0, border_width=1)
        new_category.place(relx=0.3, rely=0.37)
        
        ctk.CTkLabel(add_frame, text=f"{("Lesson To Add:").upper()}", font=(font_family[0], 13)).place(relx=0.01, rely=0.56)
        new_lesson = ctk.CTkEntry(add_frame, placeholder_text="Node.js", corner_radius=0, border_width=1)
        new_lesson.place(relx=0.3, rely=0.56)
