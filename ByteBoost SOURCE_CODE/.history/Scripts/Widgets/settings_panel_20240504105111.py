...
class Lessons_panel():
    def __init__(self, master):
        ...
        ctk.CTkLabel(del_frame, text=f"{("Wanna Add a new Lesson to track").title()}", font=(font_family[2], 13)).place(relx=0.01, rely=0.01)
        ...
        ctk.CTkLabel(del_frame, text=f"{("New category:").upper()}", font=(font_family[0], 13)).place(relx=0.01, rely=0.39)
        new_category = ctk.CTkEntry(del_frame, placeholder_text="Category not Enlisted !!!", corner_radius=0, border_width=1)
        new_category.place(relx=0.3, rely=0.39)
        
        ctk.CTkLabel(del_frame, text=f"{("Lesson To Add:").upper()}", font=(font_family[0], 13)).place(relx=0.01, rely=0.58)
        new_lesson = ctk.CTkEntry(del_frame, placeholder_text="Node.js", corner_radius=0, border_width=1)
        new_lesson.place(relx=0.3, rely=0.58)
