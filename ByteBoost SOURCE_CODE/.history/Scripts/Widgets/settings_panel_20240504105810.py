...
class Lessons_panel():
    def __init__(self, master):
        ...
        ctk.CTkCheckBox(del_frame, text="", border_width=1,checkbox_width=20,checkbox_height=20, corner_radius=0).place(relx=0.4, rely=0.39)
        
        ctk.CTkLabel(del_frame, text=f"{("Lesson To Add:").upper()}", font=(font_family[0], 13)).place(relx=0.01, rely=0.58)
        new_lesson = ctk.CTkEntry(del_frame, placeholder_text="Node.js", corner_radius=2, border_width=1)
        new_lesson.place(relx=0.3, rely=0.6)
