...
class Lessons_panel():
    ...
    def create_del_frame(self, main_frame):
        ...
        titleVar = ctk.StringVar(value=category_list[0])
        ctk.CTkLabel(del_frame, text=f"{("Category:").upper()}", font=(font_family[0], 13)).place(relx=0.01, rely=0.2)
        category = ctk.CTkComboBox(del_frame, corner_radius=0, values=category_list, variable=titleVar)
        category.place(relx=0.21, rely=0.2)
        ...
        save_btn = ctk.CTkButton(del_frame, text="SAVE",corner_radius=0,font=(font_family[0], 12),command= lambda: print(titleVar.get()))
        save_btn.place(anchor="ne", relx=0.99, rely=0.82, relwidth=0.1)