...
class File_panel():
    ...
    def create_import_frame(self, main_frame):
        ...
        settings_btn = Btn(import_frame,"SETTINGS", 0.01)
        settings_btn = Btn(import_frame,"LESSONS", (1/3) + 0.01)
        settings_btn = Btn(import_frame,"STATISTICS", (2/3) + 0.01)
        
        class Btn():
            def __init__(self, master,text, x):
                btn = ctk.CTkButton(import_frame, text=text, corner_radius=0, font=(font_family[0], 20))
                btn.place(relx=x, rely=0.8, relwidth=(1/3), relheight=(3/4), anchor="sw")
