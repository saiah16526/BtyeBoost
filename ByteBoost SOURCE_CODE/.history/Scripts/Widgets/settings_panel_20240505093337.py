...
class File_panel():
    ...
    def create_import_frame(self, main_frame):
        ...
        class Btn():
            def __init__(self, master,text, x):
                btn = ctk.CTkButton(import_frame, text=text, corner_radius=0, font=(font_family[0], 20))
                btn.place(relx=x, rely=0.94, relwidth=(1/3), relheight=(3/4), anchor="sw")
                
        settings_btn = Btn(import_frame,"SETTINGS", 0.005)
        lessons_btn = Btn(import_frame,"LESSONS", (1/3))
    ...
    def create_export_frame(self, main_frame):
        ...
        class Btn():
                def __init__(self, master,text, x):
                    btn = ctk.CTkButton(master, text=text, corner_radius=0, font=(font_family[0], 20))
                    btn.place(relx=x, rely=0.94, relwidth=(1/3), relheight=(3/4), anchor="sw")
                    
        settings_btn = Btn(export_frame,"SETTINGS", 0.005)
        lessons_btn = Btn(export_frame,"LESSONS", (1/3))
        stats_btn = Btn(export_frame,"STATISTICS", (2/3))
