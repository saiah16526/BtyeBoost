...
class Settings(ctk.CTkFrame):
    def __init__(self, master):
        ...
        self.create_buttons(frame, text="RESET", frame=file)
    ...
    def create_buttons(self, master, text, frame = None):
        self.theme_btn = ctk.CTkButton(master,corner_radius=0, text=text, font=(font_family[0], 20), command=lambda: self.bring_frame_to_front(frame.main_frame))
        self.theme_btn.pack(expand=True, fill="both", pady=1)
        
    def bring_frame_to_front(self, frame):
        frame.lift()
        frame.after(1000, self.bring_frame_to_front)
    def bring_self_to_front(self):
        self.lift()
        self.after(1000, self.bring_frame_to_front)
