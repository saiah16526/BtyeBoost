...
class Theme_panel():
    def __init__(self, master):
        ...
        self.Theme_wrapper(self.main_frame,"theme", 0.005)
        self.Theme_wrapper(self.main_frame,"theme", (1/2))
        
    class Theme_wrapper():
        def __init__(self, master,text, x):
            theme_1 = ImageTk.PhotoImage(Image.open("./Images/theme_1.png").resize((200,130)))
            frame = ctk.CTkFrame(master, fg_color="blue")
            frame.place(relx=x, rely=0.01, relwidth=(1/2) - 0.015, relheight=(1/2)-0.15)
            
            ctk.CTkLabel(frame, image=theme_1, text="").place(relx=0.001, rely=0.001, relwidth=0.99, relheight=0.99)
            ctk.CTkRadioButton(
                frame,
