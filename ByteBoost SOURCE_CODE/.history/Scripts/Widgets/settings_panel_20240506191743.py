...
class Settings(ctk.CTkFrame):
    ...
    def reset_function(self):
        answer = messagebox.askyesno(title="RESET", message="ARE YOU SURE YOU WANNA RESET EVERYTHING", fg_color=self.SETTINGS["BACKGROUND"])
        if answer:
    ...
    def create_import_frame(self, main_frame):
        ...
        class Btn():
            def __init__(self, master,text, x, command=lambda: print("debug")):
                self.SETTINGS = load_settings()
                btn = ctk.CTkButton(import_frame, text=text, corner_radius=0, font=(self.SETTINGS["FONT_FAMILY"][0], 20), command=command)
