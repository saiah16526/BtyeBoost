...
class Lessons_panel():
    ...
    def create_add_frame(self, main_frame):
        ...
        ctk.CTkLabel(add_frame, text=f"{("Category:").upper()}", font=(self.SETTINGS["FONT_FAMILY"][0], 13)).place(relx=0.01, rely=0.2)
        category = ctk.CTkComboBox(add_frame,values=category_list, corner_radius=0)
        category.place(relx=0.21, rely=0.2)
        
        ctk.CTkLabel(add_frame, text=f"{("New category:").upper()}", font=(self.SETTINGS["FONT_FAMILY"][0], 13)).place(relx=0.01, rely=0.39)
        ...
        ctk.CTkLabel(add_frame, text=f"{("Lesson To Add:").upper()}", font=(self.SETTINGS["FONT_FAMILY"][0], 13)).place(relx=0.01, rely=0.58)
    ...
    def create_del_frame(self, main_frame):
        ...
        ctk.CTkLabel(del_frame, text=f"{("Category:").upper()}", font=(self.SETTINGS["FONT_FAMILY"][0], 13)).place(relx=0.01, rely=0.2)
        ...
        ctk.CTkLabel(del_frame, text=f"{("Just The category:").upper()}", font=(self.SETTINGS["FONT_FAMILY"][0], 13)).place(relx=0.01, rely=0.39)
        ...
        ctk.CTkLabel(del_frame, text=f"{("Lesson To DElete:").upper()}", font=(self.SETTINGS["FONT_FAMILY"][0], 13)).place(relx=0.01, rely=0.58)
    ...
    def create_import_frame(self, main_frame):
        ...
        class Btn():
            def __init__(self, master,text, x, command=lambda: print("debug")):
                self.SETTINGS = load_settings()
                btn = ctk.CTkButton(import_frame, text=text,fg_color=self.SETTINGS["BACKGROUND"],hover_color=self.SETTINGS["BORDER_COLOR"], corner_radius=0, font=(self.SETTINGS["FONT_FAMILY"][0], 20), command=command)
    ...
    def create_export_frame(self, main_frame):
        ...
        class Btn():
                def __init__(self, master,text, x, command = lambda: print("here")):
                    self.SETTINGS = load_settings()
                    btn = ctk.CTkButton(master, text=text,fg_color=self.SETTINGS["BACKGROUND"],hover_color=self.SETTINGS["BORDER_COLOR"],  corner_radius=0, font=(self.SETTINGS["FONT_FAMILY"][0], 20), command=command)
                    btn.place(relx=x, rely=0.94, relwidth=(1/3), relheight=(3/4), anchor="sw")
