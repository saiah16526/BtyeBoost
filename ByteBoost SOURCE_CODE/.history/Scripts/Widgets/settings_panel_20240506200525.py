...
class Settings(ctk.CTkFrame):
    ...
    def reset_function(self):
        ...
        if answer:
            with open("./Settings/reset.json" , "r") as reset_file:
                with open ("./SaveFiles/lessons.json", "+r") as lesson_file:
                    lesson_file.seek(0)
                    lesson_file.write(json.dumps(json.load(reset_file), indent=2))
                    lesson_file.truncate()
                with open("./SaveFiles/Stats.json", "w") as stats_file:
                    stats_file.seek(0)
...
class Theme_panel():
    def __init__(self, master):
        ...
        def theme_wrapper(master, x, y,details, path):
            theme_1 = ImageTk.PhotoImage(Image.open(path).resize((200,150)))
