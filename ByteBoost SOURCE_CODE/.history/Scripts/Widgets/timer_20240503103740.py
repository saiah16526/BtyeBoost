...
class TimerFrame(ctk.CTkFrame):
    ...
    def start_timer(self):
        if not self.timer_running:
            ...
            self.clock.configure(text=str(self.remaining_time)[:-3])
    ...
    def update_timer(self):
        if self.timer_running and self.remaining_time.total_seconds() > 0:
            ...
            self.progress_meter()
            self.clock.configure(text=str(self.remaining_time))
            self.timer_id = self.after(1000, self.update_timer)
        else:
            self.stop_timer()

    def progress_meter(self):
        # Function to update the progress canvas
