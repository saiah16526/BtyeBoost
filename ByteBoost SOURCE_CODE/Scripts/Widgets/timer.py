from Scripts.Widgets.mini_stats import StatsWrapper
from Scripts.Widgets.mega_stats import Statistics
from modules import *
...
class TimerFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.previous_total_minutes = None
    ...
    def create_widgets(self):
        ...
        ctk.CTkLabel(self, text_color=self.SETTINGS["PROGRESS_THEME"], textvariable=self.lang, font=(self.SETTINGS["FONT_FAMILY"][2], 14), anchor="w").place(relx=0.42, rely=0.37)
        ctk.CTkLabel(self, text_color=self.SETTINGS["BORDER_COLOR"], text="Desired Duration:", font=(self.SETTINGS["FONT_FAMILY"][2], 14), anchor="w").place(relx=0.42, rely=0.52)

        self.entry_value = ctk.StringVar()
        durations = ["0:30", "1:00", "1:45", "2:00", "3:00", "5:00", "8:00"]
        ctk.CTkComboBox(self, variable=self.entry_value, corner_radius=3, values=durations).place(relx=0.82, rely=0.48, relwidth=0.16)

        #Images
        start_img = ImageTk.PhotoImage(Image.open(resource_path(".\\Images\\Icons\\start.png")).resize((50, 50)))
        pause_img = ImageTk.PhotoImage(Image.open(resource_path(".\\Images\\Icons\\pause.png")).resize((50, 50)))
        reset_img = ImageTk.PhotoImage(Image.open(resource_path(".\\Images\\Icons\\reset.png")).resize((50, 50)))
        # Buttons
    ...
    def start_timer(self):
        if not self.timer_running:
            ...
            if self.previous_remaining_time is None:
                ...
                self.total_minutes = self.previous_total_minutes
    ...
    def pause_timer(self):
        if self.timer_running:
            ...
            self.previous_total_minutes = self.total_minutes
    ...
    def stop_timer(self):
        ...
        if self.timer_running or self.timer_paused:
            ...
            with open(resource_path(".\\SaveFiles\\lessons.json"), '+r') as rf:
            ...
            def notify():
                duration = ((self.total_minutes * 60) - self.remaining_time.total_seconds()) / 3600
                messagebox.showinfo(title="COMPLETED", message=f'''LESSON: {self.lang.get()}\nDURATION: {duration} Hours''')
